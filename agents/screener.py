"""
agents/screener.py  (Day 5 — updated with Task MCP)
══════════════════════════════════════════════════════════════════
Now calls:
  - Task MCP → creates screening review tasks for HR
  - Email MCP → sends rejection emails to rejected candidates
"""
...
from mcp.task_manager_mcp import create_screening_tasks
from mcp.email_mcp import send_rejection_email
...
async def screener_node(state: HiringState) -> dict:
    ...
    # ── Task MCP → create HR review task ─────────────────────────────────────
    await create_screening_tasks(
        job_id          = state["job_id"],
        candidate_count = len(candidates),
    )

    # ── Email MCP → send rejection emails ────────────────────────────────────
    for c in rejected:
        ...
        logger.info("[Screener] Rejection email sent to %s", c["email"])
